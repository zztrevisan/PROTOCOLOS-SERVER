...
async function enviarNotificacaoNovoProtocolo({ protocolo, itens, destinatario }) {
  ...
  const resposta = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: remetente,
      to: emails,
      subject: `Você tem 1 nova solicitação de entrega - Protocolo ${numero}`,
      html: `
        <div style="font-family:Arial,sans-serif;color:#172b45;line-height:1.55;max-width:620px">
          <h2 style="margin-bottom:4px">Você tem 1 nova solicitação de entrega</h2>
...
async function enviarAlertaVencimentos({ destinatario, nomeEntregador, itens }) {
  ...
  const linhas = (itens || []).map(item => {
    ...
    const data = String(item.vencimento || '').split('-').reverse().join('/');
    const prazo = Number(item.dias_restantes) === 1
      ? 'vence amanhã'
      : `vence em ${item.dias_restantes} dias`;
    return `<li style="margin-bottom:10px"><strong>Protocolo ${numero} · ${escaparHtml(item.cliente)}</strong><br>${escaparHtml(item.descricao)} · ${data} (${prazo})</li>`;
  }).join('');
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;color:#172b45;line-height:1.55;max-width:650px">
          <h2>Documentos próximos do vencimento</h2>
