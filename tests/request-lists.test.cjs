...
test('Entregues não aparecem em solicitações, nem por número; excluir fica restrito ao admin em entregues',()=>{
  ...
  const scope={$:key=>fields[key],requests:[protocol],protocols:[protocol],visibleRequestsForUser:()=>[protocol],protocolMatchesSearch:()=>true,atualizarBarraEtiquetas(){},isAdmin:()=>admin,canPrintLabels:()=>admin,esc:value=>String(value||''),fmtNum:value=>String(value).padStart(6,'0'),statusBadge:value=>value,etiquetasSelecionadas:new Set()};vm.createContext(scope);
  ...
  admin=false;scope.renderCompleted();assert.ok(!fields.completedList.innerHTML.includes('deleteProtocol('));assert.ok(!fields.completedList.innerHTML.includes('Selecionar etiqueta'));
});
